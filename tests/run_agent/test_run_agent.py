...
def test_aiagent_uses_cursor_agent_acp_client():
    with (
        patch("run_agent.get_tool_definitions", return_value=_make_tool_defs("web_search")),
        patch("run_agent.check_toolset_requirements", return_value={}),
        patch("run_agent.OpenAI") as mock_openai,
        ...
        agent = AIAgent(
            ...
            acp_command="/usr/local/bin/agent",
            acp_args=["acp"],
            quiet_mode=True,
            skip_context_files=True,
            skip_memory=True,
    ...
    assert mock_acp_client.call_args.kwargs["command"] == "/usr/local/bin/agent"
    assert mock_acp_client.call_args.kwargs["args"] == ["acp"]
    assert mock_acp_client.call_args.kwargs["backend_label"] == "Cursor Agent ACP"
